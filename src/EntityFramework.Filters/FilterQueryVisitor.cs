    ...
    public class FilterQueryVisitor : DefaultExpressionVisitor
    {
        ...
        public override DbExpression Visit(DbScanExpression expression)
        {
            try
            {
                ...
                if (!annotations.Any())
                {
                    Console.WriteLine("Didn't find any annotations. Aborting visitor");
                    return base.Visit(expression);
                }
                ...
                foreach (var globalFilter in annotations.Where(a => a.Key.StartsWith("globalFilter")))
                {
                    ...
                    Console.WriteLine("Found Filter: {0}", convention.Name);

                    Filter filterConfig;

                    string filterName = globalFilter.Key.Split(new[] {"globalFilter_"}, StringSplitOptions.None)[1];

                    if (!FilterExtensions.FilterConfigurations.TryGetValue(
                        new Tuple<string, object>(filterName, _contextForInterception.GetInternalContext()),
                        out filterConfig))
                    {
                        Console.WriteLine("Didn't find FilterConfiguration");
                        _contextForInterception.WhatFiltersDoIHave();
                        continue;
                    }
                    ...
                    {
                        Console.WriteLine("Filter {0} is disabled", filterConfig.FilterName);
                        continue;
                    }
                    ...
                    Console.WriteLine("Applied the filter");
                }
                return current;
            }
            catch (Exception)
